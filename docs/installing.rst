...
This is the documentation for sonicord, a library for Python to aid
in creating applications that utilise the Discord API.

Prerequisites
-------------

sonicord works with Python 3.8 or higher. Support for earlier versions of Python
...
To install additional packages for speedup,  you should use ``sonicord[speed]`` instead of ``sonicord``, e.g.
...
To get voice support, you should use ``sonicord[voice]`` instead of ``sonicord``, e.g. ::
...
sonicord revolves around the concept of :ref:`events <discord-api-events>`.
